...
Based on the summary statistics, for the fare amount, the mean fare amount is 18.59 USD, while the median is 12.8 USD, which again points to a right-skewed distribution for this column. This confirms what we saw with the density chart.
...
The regression line formula is: y_hat = `{python} slope` * trip_distance + `{python} intercept`
...
The linear regression model shows moderate predictive power with an R-squared value of `{python} r2_score(y_true, y_pred)`, which explains about high percent of fare variance. The low MAE of `{python} mean_absolute_error(y_true, y_pred)` USD indicates good accuracy for typical rides, although the higher RMSE (`{python} np.sqrt(mean_squared_error(y_true, y_pred))` USD) suggests sensitivity to outliers. The visual analysis of the errors reveals the model performs best for the 10-50 USD range, and underestimates fares over 100 USD. The actual NYC taxi fare is calculated through initial fare, per-mile charge, and additional fees for peak hours, nighttime, tolls, etc that this model cannot predict, but can be built further using more complicated multilinear regression or ML models.

Based on the estimated result from the regression line formula derived from the model, this suggests that for each additional mile traveled, the fare increases by approximately `{python} slope` USD, and the base fare (when the trip distance is zero) is around `{python} intercept` USD.
